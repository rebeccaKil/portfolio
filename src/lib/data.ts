...
export const HERO = {
  ...
  subtext:
    "트래블 커머스에서 결제·예약 경험을 개선하는 Product Owner입니다. 타이드스퀘어(글로벌 트레블테크 OTA · MAU 100만)에서 모바일웹 결제 백지화면 이슈를 UX 관점으로 풀어 결제 단계 이탈을 일 평균 104건 → 30건(약 71%)으로 줄였고, 통합결제 구축으로 결제 버튼 클릭률을 69% → 74%(+5%p)로 끌어올렸습니다.",
  stats: [
