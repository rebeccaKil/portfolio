...
export const HERO = {
  name: "Rebecca K.",
  role: "Product Owner",
  domain: "Travel & E-commerce · Payments · AI Automation",
  headline: ["예약·결제·어드민 구조를", "개선하고, 전환율로", "증명합니다"],
  subtext:
    "여행·이커머스 플랫폼에서 예약·결제·어드민 구조를 개선하고, 데이터와 AI 자동화로 전환율과 운영 효율을 높이는 Product Owner입니다. 타이드스퀘어(MAU 100만)에서 결제 백지화면 개선으로 결제 단계 이탈을 71% 줄이고(일 104→30건), 통합결제 구축으로 예약 완료율 +3%p(86→89%)·연 약 27억 원 추가매출을 만들었습니다. 5인 CX PO팀을 리딩한 PO 리더입니다.",
  stats: [
...
export type CaseSection =
  ...
  | { type: "roles"; scope: string; decided: string; coordinated: string }
  ...
  // ── Case 01 ──────────────────────────────────────────────────────────────
  {
    ...
    sections: [
      ...
      },
      {
        type: "highlight",
        eyebrow: "문제",
        body: "투어비스 메인은 배너·카테고리·팝업·큐레이션이 모두 코드에 하드코딩되어 있었습니다. 문구 하나를 바꿔도 프론트 배포가 필요했고, 호텔·투어·항공 카테고리마다 같은 기능을 중복 개발했습니다. 마케팅 시점과 배포 일정이 어긋나며 운영과 개발 모두의 병목이 됐습니다.",
      },
      {
        type: "highlight",
        eyebrow: "해결",
        body: "모든 전시 유형을 '이미지 + 링크 + 정렬 + 기간'이라는 공통 구조(인벤토리)로 추상화하고, 메인 전체를 한 번의 호출로 내려주는 Page API를 설계했습니다. 운영자는 어드민에서 구좌를 블록처럼 조립하고, 프론트는 어떤 카테고리든 같은 방식으로 그립니다.",
      },
      {
        type: "roles",
        scope: "투어비스 메인 전시 전 영역(배너·카테고리·팝업·큐레이션). CX PO 팀장으로 8개월 리뉴얼 총괄.",
        decided: "전시 유형을 공통 구조로 추상화하고 메인을 Page API 단일 호출로 통합. 구좌 구성 권한을 코드에서 어드민으로 이관.",
        coordinated: "FE 3·BE 2·디자인 1과 API 스펙·어드민 구조·QA 기준 합의. 마케팅·운영의 개별 구좌 요구를 공통 스키마로 수렴.",
      },
      {
        type: "flow",
      ...
      },
      {
        type: "tags",
        label: "핵심 역량",
        items: ["정책 설계", "정보구조 설계", "어드민 기획", "API 설계 협업", "QA 기준"],
      },
    ],
  },
  ...
  // ── Case 02 — 모바일웹 결제 백지화면 개선 ──────────────────────────────
  {
    ...
    sections: [
      ...
      },
      {
        type: "highlight",
        eyebrow: "문제",
        body: "모바일웹 결제 단계에서 응답 대기 중 백지 화면이 노출되는 이슈가 있었습니다. 사용자는 이를 오류로 인지하고 이탈했고, 결제 단계에서 이탈이 발생한 예약 건수가 일 평균 104건으로 집계됐습니다.",
      },
      {
        type: "pullquote",
      ...
      },
      {
        type: "highlight",
        eyebrow: "해결",
        body: "로딩·대기 화면 정책을 정의하고(노출 조건 · 최대 대기 시간 · 타임아웃 시 안내 문구), 스펙을 작성해 개발·QA와 협업으로 배포했습니다. 결과: 결제 단계 이탈 예약 건수 일 평균 104건 → 30건, 약 71% 감소.",
      },
      {
        type: "roles",
        scope: "모바일웹 결제 단계 이탈 개선. 문제 정의부터 정책·스펙·배포·검증까지.",
        decided: "이탈 원인을 '속도'가 아닌 '인지'로 재정의. 응답속도 개선(A) 대신 로딩·대기 화면(B)을 우선 적용하고, 속도 과제는 QA·NPS 데이터로 종결.",
        coordinated: "개발·QA와 로딩 노출 조건·최대 대기·타임아웃 문구를 합의하고, 배포 후 QA·NPS로 재발 여부를 함께 검증.",
      },
      {
        type: "insight",
        label: "회고",
      ...
      },
      {
        type: "tags",
        label: "핵심 역량",
        items: ["퍼널 분석", "문제 재정의", "정책 설계", "개발 협업", "QA"],
      },
    ],
  },
  ...
  // ── Case 03 — 통합결제 구축 ─────────────────────────────────────────────
  {
    ...
    sections: [
      ...
      },
      {
        type: "highlight",
        eyebrow: "문제",
        body: "상품군별로 결제 방식이 분리되어 있었습니다. 결제 수단이 파편화되고, 같은 정책(쿠폰·취소·환불)을 카테고리마다 따로 유지보수해야 했으며, 결제 진입 경험이 카테고리마다 달라 사용자 혼란이 누적됐습니다.",
      },
      {
        type: "highlight",
        eyebrow: "해결",
        body: "통합결제 정책을 정의하고 요구사항·스펙을 작성했으며, 예약-결제 통합 아키텍처 설계에 개발팀과 협업했습니다. 쿠폰 적용 순서·중복 규칙·취소·환불 플로우를 처음으로 문서화해 전 카테고리가 같은 결제 레이어를 쓰게 했습니다.",
      },
      {
        type: "highlight",
        eyebrow: "왜 이렇게",
        body: "카테고리마다 결제를 따로 만들면 신규 카테고리·채널이 추가될 때마다 결제를 재개발해야 합니다. 결제를 공통 레이어로 분리하면 한 번 정의한 정책이 전 카테고리에 반복 적용됩니다 — 확장 비용을 낮추는 것이 목적이었습니다.",
      },
      {
        type: "roles",
        scope: "통합결제·쿠폰 v2·취소·환불 전 영역. 결제를 카테고리에서 분리하는 구조 설계.",
        decided: "결제를 공통 레이어로 분리. 쿠폰 적용 순서(TV포인트 선차감)·중복 규칙·환불 정산 기준을 확정.",
        coordinated: "BE 3·FE 2와 예약-결제 아키텍처·ERD 리뷰. 마케팅·정산·CS의 쿠폰 조건을 하나의 매트릭스로 통합.",
      },
      {
        type: "flow",
      ...
      },
      {
        type: "tags",
        label: "핵심 역량",
        items: ["결제 기획", "정책 설계", "정산 정합성", "API·ERD 리뷰", "QA"],
      },
    ],
  },
  ...
  // ── Case 04 ────────────────────────────────────────────────────────────
  {
    ...
    sections: [
      ...
      },
      {
        type: "highlight",
        eyebrow: "문제",
        body: "투어·티켓 상품은 탐색은 길고 결정은 나중입니다. 그런데 관심 상품을 담아둘 방법이 없어 탐색과 구매 사이 구간이 끊겨 있었고, 여러 상품을 한 번에 결제할 방법도 없어 상품마다 결제를 반복해야 했습니다.",
      },
      {
        type: "highlight",
        eyebrow: "해결",
        body: "탐색 저장(찜: 상품 ID)과 구매 준비(장바구니: 상품+이용일+옵션+인원)를 다른 저장 단위로 분리해 설계하고, 여러 상품을 PG 1회 승인으로 결제하되 상품별로 예약·환불이 독립되는 통합 아키텍처를 정의했습니다. 구매 행동 전체는 Motivation·Ability·Reward 세 축으로 설계했습니다.",
      },
      {
        type: "roles",
        scope: "찜·장바구니 신규 플로우와 포인트 리워드. 기획 완료, 개발 설계 단계(2026.09 오픈 목표).",
        decided: "찜(상품 ID)과 장바구니(상품+이용일+옵션+인원) 저장 단위 분리. PG 1회 승인·상품별 독립 예약/환불 구조 채택. KPI 5종 정의.",
        coordinated: "개발과 예약-결제 통합 아키텍처를 논의하고, Claude Code로 기존 구현 코드를 분석해 PRD를 자동 생성 — 기획과 구현의 정합성 확보.",
      },
      {
        type: "grid3",
        items: [
      ...
      },
      {
        type: "tags",
        label: "핵심 역량",
        items: ["행동 설계(M·A·R)", "정보구조 설계", "KPI 정의", "AI 기획 자동화", "개발 협업"],
      },
    ],
  },
  ...
  // ── Case 05 ──────────────────────────────────────────────────────────────
  {
    ...
    sections: [
      ...
      },
      {
        type: "highlight",
        eyebrow: "문제",
        body: "AI를 업무에 적극 활용하기 위해 신설된 AX 유닛으로 이동하며, 기존에 담당하던 결제·회원·전시·투티·쿠폰 5개 영역을 그대로 안고 왔습니다. 영역마다 과제가 산재해 무엇을 먼저 할지 정하는 기준이 필요했고, 버스투어 서비스 종료로 기존 유입 수요가 갈 곳을 잃은 상태였습니다.",
      },
      {
        type: "highlight",
        eyebrow: "해결",
        body: "과제를 5개 OKR로 묶어 월 73건+ 티켓의 우선순위를 결정했습니다. 버스투어 수요는 노출·클릭·구매 전환 데이터를 분석해 실제 구매가 발생하는 대체 상품 추천군으로 이관했고, 반복 운영 업무는 자동화로 제거해 신규 과제에 리소스를 확보했습니다.",
      },
      {
        type: "roles",
        scope: "결제·회원·전시·투티·쿠폰 5개 영역, OKR 5개·월 73건+ 티켓.",
        decided: "매출·고객 불편·운영 리스크·개발 리소스 기준으로 티켓 우선순위 결정. 버스투어 수요를 데이터 기반 대체 추천군으로 이관.",
        coordinated: "개발·디자인·CS와 OKR 정렬. 배포는 개발팀 확인을 거쳐 진행하고, Slack 장애 알림으로 배포 후 모니터링.",
      },
      {
        type: "grid2",
        items: [
      ...
      },
      {
        type: "tags",
        label: "핵심 역량",
        items: ["OKR 운영", "퍼널 분석", "다이나믹 프라이싱", "운영 자동화", "AI 활용 배포"],
      },
    ],
  },
];
...
// ─── TEAM LEADERSHIP ─────────────────────────────────────────────────────────
export const TEAM = {
  eyebrow: "CX PO 팀장 · 2021.11 — 2025.01",
  title: "5명의 팀을 이끌고 CX 전 영역을 관리하다",
  paragraphs: [
    "시니어 1명과 주니어 4명, 총 5명의 CX PO팀을 리딩하며 결제·회원·전시·검색·고객센터·리뷰 도메인을 분담 운영했습니다. 팀원 각각이 도메인을 전담하고, 팀장으로서 모든 도메인의 프로젝트 일정을 관리하고 기획 산출물을 검수했습니다. 스펙 리뷰를 거쳐 오픈 전 정책 누락을 줄였고, 주니어 4명 모두 담당 도메인 프로젝트를 단독 오픈까지 리드하도록 성장시켰습니다.",
    "백로그 우선순위는 매출 영향도·고객 불편도·운영 리스크·개발 리소스 네 가지 기준으로 정렬했습니다. 전사 Jira 보드를 통합해 PO–개발–QA–CS 간 업무 흐름을 표준화하고, 요청 접수부터 배포·CS 대응까지 하나의 흐름에서 추적되도록 만들었습니다.",
    "팀 목표는 OKR 단위로 수립하고 운영했습니다. 외부 채널 전환율 확대, 다이나믹 프라이싱·쿠폰 메커니즘, 운영 자동화, 검색·SEO 노출, 모니터링 대시보드 — 5개 OKR 아래 월간 70건 이상의 티켓을 관리하며 방향을 정렬했습니다.",
...
export const CAREER = [
  {
    ...
    desc: "AI를 업무에 적극 활용하기 위해 신설된 AX 유닛의 PO. 기존 담당인 결제·회원·전시·투티·쿠폰 전 영역을 이어서 담당. 다이나믹 프라이싱(USJ), 선착순 쿠폰, 찜·장바구니 신규 설계, 투어비스 포인트 리워드 시스템. OKR 기반 월 73건+ 티켓 운영. 버스투어 전환 데이터 분석(CTR 5.7%, 구매전환 4.6%)으로 추천군 재편. Cursor·Codex·Gemini·Claude Code로 4개 페이지 직접 배포.",
  },
  {
    period: "약 6개월",
  ...
  },
  {
    ...
    company: "하이퍼클라우드 (Hypercloud)",
    role: "PO · 개발 PM · AR/VR 스마트관광 플랫폼",
    desc: "터치수원 프로젝트 개발 PM, AR/VR 기능 개발 PM — FP 산정, 과업지시서 작성, 일정관리, 업무 조율. AR/VR 기반 소상공인 상품 판매 시스템 기획.",
  },
  {
    period: "2017.12 — 2021.03",
    company: "줌줌투어 (ZoomZoom Tour)",
    role: "BD · PO · 투어&티켓",
  ...
  {
    period: "2011.04 — 2017.12",
    company: "PLAY.D",
    role: "Marketing Manager",
    desc: "100여개 업체(숙박·부동산·자동차·병원·쇼핑몰) 포털 SA/DA·배너·SNS·앱·동영상 광고 집행, 홈페이지 UI 기획 및 SEO 최적화. 마케팅 전략·비즈니스 관계 관리.",
  },
  {
    period: "2010.01 — 2011.04",
    company: "네이버 (NAVER Corp)",
    role: "AE · 검색마케팅",
    desc: "검색광고 기획·운영. 첫 직장, 데이터와 전환을 제품 판단의 언어로 다루는 기반.",
  },
];
