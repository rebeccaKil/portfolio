...
export const HERO = {
  ...
  subtext:
    "트래블 커머스에서 결제·예약 경험을 개선하는 Product Owner입니다. 타이드스퀘어(글로벌 트레블테크 OTA · MAU 100만)에서 모바일웹 결제 백지화면 이슈를 UX 관점으로 풀어 결제 단계 이탈을 일 평균 104건 → 30건(약 71%)으로 줄였고, 통합결제 구축으로 결제 버튼 클릭률을 69% → 74%(+5%p), 예약 완료율을 86% → 89%(+3%p)로 끌어올렸습니다.",
  stats: [
...
export const CASES: CaseStudy[] = [
  ...
  // ── Case 02 — 모바일웹 결제 백지화면 개선 ──────────────────────────────
  {
    ...
    sections: [
      ...
      },
      {
        type: "insight",
        label: "회고",
        body: "문제를 '속도'가 아닌 '인지'로 재정의해 최소 비용으로 최대 효과를 냈습니다. 응답 대기 시간 자체의 단축은 중장기 과제로 분리해 후속 진행했습니다.",
      },
    ],
  },
  ...
  // ── Case 04 ────────────────────────────────────────────────────────────
  {
    ...
    sub: "찜·장바구니 신규 설계 · 포인트 리워드 · KPI 정의 · Claude Code PRD 자동화 · 2024 — 현재",
    accent: "#10b981",
    sections: [
      {
        type: "impacts",
  ...
  // ── Case 05 ──────────────────────────────────────────────────────────────
  {
    ...
    sections: [
      ...
      },
      {
        ...
        body: "Cursor·Claude Code로 인터랙티브 목업·프로토타입을 직접 만들어 개발 착수 전에 정책과 엣지케이스를 검증하고, 개발·디자인과의 논의 사이클을 단축합니다. Bitbucket 브랜치 관리·코드 리뷰, Vercel 배포, Supabase PostgreSQL DB 구축까지 — 사내 도구 수준의 서비스는 직접 구현·배포한 경험이 있습니다.",
      },
      {
        type: "images",
        label: "산출물",
...
export const ABOUT = {
  intro: [
    ...
    "AI 도구로 검증과 커뮤니케이션의 속도를 높입니다. Cursor·Claude Code로 인터랙티브 목업·프로토타입을 직접 만들어 개발 착수 전에 정책·엣지케이스를 검증하고, 개발·디자인과의 논의 사이클을 단축합니다. 내부 서비스 DB와 GA를 MCP로 연동해 데이터를 즉시 확인하고, Jira·Gmail 자동화로 업무 현황을 Confluence에 적재하며, Slack 봇으로 반복 업무를 줄입니다. 사내 도구 수준의 서비스는 직접 구현·배포한 경험이 있습니다.",
  ],
  highlights: [
    ...
    { icon: "⬛", label: "AI로 검증·협업 가속", desc: "목업·프로토타입으로 개발 착수 전 정책 검증. DB·GA MCP 연동, Jira·Gmail·Slack 자동화. 사내 도구는 직접 구현·배포." },
  ],
  validation: {
    grade: "A+",
